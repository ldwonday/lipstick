...
export default class extends PureComponent {
  ...
  render() {
    ...
    return <block>
        {isShow && <View className="dialog dialog-share" onTouchMove={this.handleOnTouchMove.bind(this)}>
            <View className="dialog-box">
              <Image onClick={onClose} className="close" mode="aspectFill" src={closeBtn} />
              <Text className="share-title">分享有礼</Text>
              <View className="share-blocks">
                {shareListArray && shareListArray.map((item, index) => {
              return <View className="item" key={index}>
                        <Form onSubmit={onSaveForm} reportSubmit="true">
                          {item.sk_id && <View className="inner">
                          ...
                          {!item.sk_id && <Button className="inner" openType="share" data-code={shareCode} formType="submit">
                              <Text className="name">
